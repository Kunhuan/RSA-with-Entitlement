...
def clear_old_results():
    """清理旧结果文件"""
    old_files = list(RSA_OLD_RESULTS_DIR.glob("rsa_old_evaluation_*.csv"))
...
class GPUCorrectedRSA:
    ...
    def __init__(self, meaning_space, utterance_space, cluster_labels, costs_df, utterance_ids, alpha=0.8):
        self.device = device
        self.alpha = alpha
    ...
    def pragmatic_listener(self, utterance_idx, prior):
        ...
        return joint_probs / total_prob if total_prob > 0 else prior.clone()
    
    def process_conversation_sequence(self, utterance_indices):
        """处理对话序列（修正版）
        
        RSA模型的计算流程：
        1. 第一轮：使用全局先验，计算B对A第一个utterance的语用听者信念
        2. 后续轮次：使用全局先验，计算B对A当前utterance的语用听者信念
        3. 注意：RSA没有资格参数更新，所有语境信息都来自贝叶斯更新
        4. 注意：utterance_indices只包含A的utterances，不包含B的utterances
        """
        belief_sequence = [self.global_prior.clone()]
        
        for u_idx in utterance_indices:
            # RSA模型：始终使用全局先验，没有动态更新
            pragmatic_result = self.pragmatic_listener(u_idx, self.global_prior)
...
def test_gpu_corrected_rsa_old(args):
    """测试RSA模型（无动态先验版本）"""
    clear_old_results()
    ...
    checkpoint_file = RSA_OLD_RESULTS_DIR / f"checkpoint_rsa_old_{timestamp}.csv"
    ...
    output_filename = RSA_OLD_RESULTS_DIR / f"rsa_old_evaluation_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(description="RSA模型评估（无动态先验版本）")
    parser.add_argument('--max-conversations', type=int, default=RUNTIME_PARAMS['default_max_conversations'], 
                       help=f'最大处理对话数量，0表示处理全部对话 (默认: {RUNTIME_PARAMS["default_max_conversations"]})')
    parser.add_argument('--batch-size', type=int, default=RUNTIME_PARAMS['default_batch_size'],
                       help=f'批处理大小 (默认: {RUNTIME_PARAMS["default_batch_size"]})')
    args = parser.parse_args()
    
    print(f"=== RSA模型配置（无动态先验版本） ===")
    print(f"理性参数α: {RSA_PARAMS['alpha']}")
    print(f"处理对话数: {'全部' if args.max_conversations == 0 else args.max_conversations}")
    print(f"批处理大小: {args.batch_size}")
    print()
    
    test_gpu_corrected_rsa_old(args)
